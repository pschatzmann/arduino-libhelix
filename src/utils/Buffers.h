...
class RingBuffer : public BaseBuffer<T> {
 public:
  ...
  virtual void resize(int len) {
    if (max_size != len) {
      LOG_HELIX(LogLevelHelix::Info,"resize: %d", len);
...
class RingBufferFile : public BaseBuffer<T> {
 public:
  ...
  void setFile(File &bufferFile, bool clear = false) {
    p_file = &bufferFile;
    if (!*p_file) {
      LOG_HELIX(LogLevelHelix::Error,"file is not valid");
    }
    // if no clear has been requested we can access the existing data in the
    // p_file
    if (!clear) {
      element_count = p_file->size() / sizeof(T);
      LOG_HELIX(LogLevelHelix::Info,"existing elements: %s", element_count);
  ...
  T read() override {
    ...
    if (auto_rewind && isEmpty()) {
      LOG_HELIX(LogLevelHelix::Info,"pos 0");
      write_pos = 0;
      read_pos = 0;
    }
 ...
 protected:
  ...
  void file_seek(int pos) {
    if (p_file->position() != pos * sizeof(T)) {
      LOG_HELIX(LogLevelHelix::Debug,"file_seek: %d", pos);
      if (!p_file->seek(pos * sizeof(T))) {
        LOG_HELIX(LogLevelHelix::Error,"seek %d", pos * sizeof(T))
      }
    }
  }

  int file_write(const T *data, int count) {
    LOG_HELIX(LogLevelHelix::Debug,"file_write: %d", count);
    if (p_file == nullptr) return 0;
    ...
    if (bytes_written != to_write) {
      LOG_HELIX(LogLevelHelix::Error,"write: %d -> %d", to_write, bytes_written);
    }
    return elements_written;
  }

  int file_read(T *result, int count) {
    LOG_HELIX(LogLevelHelix::Debug,"file_read: %d", count);
    size_t result_bytes = p_file->read((uint8_t *)result, sizeof(T) * count);
    if (result_bytes != count * sizeof(T)) {
      LOG_HELIX(LogLevelHelix::Error,"readBytes: %d -> %d", (int)sizeof(T) * count, (int)result_bytes);
      result = 0;
    }
    return count;
  }
 ...
 public:
  ...
  void resize(int size, int count) { 
    ...
    for (int j = 0; j < count; j++) {
      ...
      if (avaliable_buffers[j] == nullptr) {
        LOG_HELIX(LogLevelHelix::Error,"Not Enough Memory for buffer %d", j);
      }
    }
  }
...
class BufferedArray {
 public:
  BufferedArray(Stream &input, int len) {
    LOG_HELIX(LogLevelHelix::Info,"BufferedArray(%d)", len);
    array.resize(len);
    p_stream = &input;
  }
  // access values, the offset and length are specified in samples of type <T>
  int16_t *getValues(size_t offset, size_t length) {
    LOG_HELIX(LogLevelHelix::Debug,"getValues(%d,%d) - max %d", offset, length, array.size());
    if (offset == 0) {
    ...
    if (size > 0) {
      LOG_HELIX(LogLevelHelix::Debug,"readBytes(%d,%d)", last_end, size);
